...
# 版本号单一真源：Info.plist 说了算，打包时自动回写 docs/version.json。
# version.json 是更新检查的三源之一，两处手改漏一处，用户就收不到更新提醒。
VERSION_JSON="docs/version.json"
JSON_VER=$(sed -n 's/.*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$VERSION_JSON")
if [ "$JSON_VER" != "$VERSION" ]; then
    sed -i '' "s/\"version\"[[:space:]]*:[[:space:]]*\"[^\"]*\"/\"version\": \"${VERSION}\"/" "$VERSION_JSON"
    echo "⚠️  $VERSION_JSON 已从 ${JSON_VER:-空} 同步到 ${VERSION}"
    echo "    发版后记得提交推送，并清 jsDelivr 缓存："
    echo "    curl -s \"https://purge.jsdelivr.net/gh/DaliangPro/ProNotch@main/docs/version.json\""
else
    echo "✅ $VERSION_JSON 版本号一致（${VERSION}）"
fi
