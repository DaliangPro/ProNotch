...
final class HookDeliveryGuardTests: XCTestCase {
    ...
    private func deliveredURL(scriptNamed name: String, watching process: String,
        ...
        let file = tmp.appendingPathComponent("\(name)-\(slug).sh")
        try script.write(to: file, atomically: true, encoding: .utf8)
        _ = try run("/bin/bash", [file.path], stdin: payload)
    ...
    func test完成提醒项目名用base64url编码() throws {
        let probe = try startProbe()
        defer { probe.process.terminate() }
        let payload = #"{"session_id":"abc","transcript_path":"/Users/x/.claude/projects/p/abc.jsonl","hook_event_name":"Stop","background_tasks":[],"cwd":"/Users/x/我的 项目"}"#
        let url = try XCTUnwrap(try deliveredURL(scriptNamed: "claude", watching: probe.name,
                                                payload: payload))
        ...
        XCTAssertEqual(AgentCompletionNotice.decodeProject(encoded), "我的 项目",
