...
/// 翻译接口弹层：地址/模型绑 SettingsStore 当前套（即时生效），Key 惰性从钥匙串读、改即写回。
/// 模型列表这一套不落盘，只在弹层里临时拉一次
struct TranslateEndpointSheet: View {
    ...
    @State private var customModels: Set<String> = []
    ...
    var body: some View {
        EndpointSheet(
            ...
            apiKey: $apiKey,
            models: listModels,
            currentModel: settings.translateModel,
            customModels: customModels,
            fetching: fetching,
            statusText: status,
            statusColor: statusColor,
            canDelete: settings.translateEndpoints.count > 1,
            onFetch: { run(populate: true) },
            onSelectModel: { settings.translateModel = $0 },
            onAddModel: { customModels.insert($0); settings.translateModel = $0 },
            onRemoveModel: { customModels.remove($0) },
            onTest: { run(populate: false) },
            onDelete: {
                if let id = settings.translateCurrentEndpointID { settings.deleteTranslateEndpoint(id) }
                dismiss()
            },
            onCancel: nil,
            onDone: { dismiss() })
        .onAppear {
            ...
            apiKey = settings.translateAPIKey()   // 只在弹层打开时碰一次钥匙串
            loadedKey = true
        }
        .onChange(of: apiKey) { _, v in
            guard loadedKey else { return }
            settings.setTranslateAPIKey(v)
        }
    }

    private var listModels: [String] {
        var seen = Set<String>()
        var out: [String] = []
        for m in Array(customModels).sorted() + models where seen.insert(m).inserted { out.append(m) }
        if !settings.translateModel.isEmpty, !seen.contains(settings.translateModel) {
            out.insert(settings.translateModel, at: 0)
        }
        return out
    }

    private var nameBinding: Binding<String> {
        Binding(get: { settings.translateEndpoints.first { $0.id == settings.translateCurrentEndpointID }?.name ?? "" },
                set: { v in
    ...
    private func run(populate: Bool) {
        ...
        let u = settings.translateBaseURL, k = apiKey
        Task {
            do {
                let m = try await ChatStore.fetchAvailableModels(baseURL: u, apiKey: k)
                if populate {
                    models = m
                    if settings.translateModel.isEmpty, let f = m.first { settings.translateModel = f }
                }
