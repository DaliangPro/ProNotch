...
struct ScreenshotPage: View {
    ...
    var body: some View {
                    ...
                    SettingsRow(title: "接口") {
                        HStack(spacing: 12) {
                            if !settings.translateUseChatAPI {
                                TextButton(title: "编辑") { endpointSheet = true }
                            }
                            PopupMenu(label: endpointLabel) {
                                ...
                                ForEach(settings.translateEndpoints) { p in
                                    Button(p.name.isEmpty ? "未命名" : p.name) {
                                        ...
                                        settings.activateTranslateEndpoint(p.id)
                                    }
                                }
                                Divider()
                                Button("新建…") {
                                    settings.translateUseChatAPI = false
                                    settings.addTranslateEndpoint()
                                    endpointSheet = true
                                }
                            }
                        }
        ...
        .sheet(isPresented: $endpointSheet) { TranslateEndpointSheet() }
    ...
    private var endpointLabel: String {
        ...
        let name = settings.translateEndpoints.first { $0.id == settings.translateCurrentEndpointID }?.name ?? ""
        return name.isEmpty ? "未命名" : name
    }
}
