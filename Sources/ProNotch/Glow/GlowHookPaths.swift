...
struct GlowHookPaths: Sendable {
    ...
    /// 旧版本装过、现已不再使用的脚本（`GlowHookInstaller.removeRetiredHooks` 清理用）
    var retiredScripts: [String] { [scriptDir + "/agent-wait.sh", scriptDir + "/agent-permission.sh"] }
    /// 旧版本的请求交换目录（同上）
    var retiredExchangeDir: String { scriptDir + "/permissions" }
