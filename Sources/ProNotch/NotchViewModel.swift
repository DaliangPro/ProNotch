...
final class NotchViewModel: ObservableObject {
    ...
    /// 「手上有活没干完」期间为 true，暂停鼠标离开触发的自动收起。
    ///
    /// **判据不能是「输入框有焦点」**。闪问的输入框发完消息还保持聚焦（方便追问），
    /// 焦点一拿到就再没放开——于是这把锁永久挂着，鼠标移开也不收，
    /// 只能按 ESC 或点别处（2026-07-28 大梁老师反馈「刘海不会收回去，都在闪问页」）。
    /// 而 `collapse()` 里的复位（见下）根本轮不到执行：收起本身就被它挡死了，是个死结。
    ///
    /// 各页自己判定「有活没干完」：闪问是「有未发出的草稿」或「AI 正在吐字」，
